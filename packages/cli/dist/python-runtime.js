...
export const getDefaultPythonRuntimeDeps = () => ({
    ...
    runCommand: (command, args, options = {}) => new Promise((resolve) => {
        let child;
        try {
...
export const getPythonProgramById = (value) => {
    const normalized = String(value || '').trim().toLowerCase();
    return PYTHON_PROGRAMS.find((program) => program.id === normalized || program.aliases.some((alias) => alias.toLowerCase() === normalized));
};
export const getPythonEnvDir = () => process.env.NBCP_PYTHON_ENV_DIR || path.join(os.homedir(), '.neuxsbot', 'python-env');
export const getVenvPythonPath = (pythonEnvDir, platform = process.platform) => (platform === 'win32' ? path.join(pythonEnvDir, 'Scripts', 'python.exe') : path.join(pythonEnvDir, 'bin', 'python'));
export const getRequirementsStampPath = (pythonEnvDir) => path.join(pythonEnvDir, '.requirements.stamp');
const getRequirementsPath = (repoRoot) => path.join(repoRoot, 'examples', 'python', 'requirements.txt');
const getPythonCandidates = async (deps) => {
    ...
    if (platform === 'win32') {
        candidates.push({ command: 'py', argsPrefix: ['-3'] }, { command: 'python', argsPrefix: [] }, { command: 'python3', argsPrefix: [] });
    }
    else {
...
const resolveWindowsCommandPath = async (commandName, deps) => {
...
const resolvePythonCommand = async (deps) => {
...
const ensureSystemPython = async (deps, onStatus) => {
    ...
    const wingetCommand = deps.env.NBCP_WINGET_BIN?.trim() || (await resolveWindowsCommandPath('winget', deps)) || 'winget';
    const installResult = await deps.runCommand(wingetCommand, ['install', '-e', '--id', 'Python.Python.3.12', '--accept-package-agreements', '--accept-source-agreements'], { inheritStdio: true });
...
const ensurePythonPackages = async (venvPythonPath, repoRoot, pythonEnvDir, deps, onStatus) => {
    ...
    const installRequirementsResult = await deps.runCommand(venvPythonPath, ['-m', 'pip', 'install', '-r', requirementsPath], {
        cwd: repoRoot,
        inheritStdio: true,
    });
...
export const ensurePythonRuntime = async (repoRoot, pythonEnvDir, deps = getDefaultPythonRuntimeDeps(), onStatus = () => { }) => {
    ...
    if (!deps.existsSync(venvPythonPath)) {
        ...
        const createVenvResult = await deps.runCommand(systemPython.command, [...systemPython.argsPrefix, '-m', 'venv', pythonEnvDir], {
            cwd: repoRoot,
            inheritStdio: true,
        });
...
export const runPythonAnalysisProgram = async (programId, options, deps = getDefaultPythonRuntimeDeps()) => {
